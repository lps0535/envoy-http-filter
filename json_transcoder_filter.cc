...
Http::FilterHeadersStatus JsonTranscoderFilter::encodeHeaders(Http::HeaderMap& headers,
  ...
  // currently hardcoding as Html, later on will change according to the content-type of HttpBody
  headers.insertContentType().value().setReference(Http::Headers::get().ContentTypeValues.Html);
